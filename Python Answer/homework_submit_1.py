...
def readInputFile(fileName):
	...
	while i < returnVal['numTrafficlines']:
		...
		if not lineList[0] in adjList:
			#adjList[lineList[0]] = {}	#for dictionary of dictionary
			adjList[lineList[0]] = []	#for dictionary of list of dictionaries
		#adjList[lineList[0]][lineList[1]] = lineList[2]	#for dictionary of dictionary
...
def getDFSOutput(inputData):
	...
	while 1:
		...
		print "Current Node : "+city
			...
			if city in inputData['adjList']:
				for adjNodes in inputData['adjList'][city]:
					print adjNodes.keys()[0]
					if (city in parentMapping and not parentMapping[city] == adjNodes.keys()[0]) or (not city in parentMapping):	#for DFS loop detection
						children.extend(adjNodes.keys())
					#pathQueue.extend(adjNodes.keys())
					parentMapping[adjNodes.keys()[0]] = city
				pathQueue = children + pathQueue
		#print parentMapping
